...
export const TradeSheet = ({
    ...
    const handleConfirm = async () => {
        ...
        if (side === 'sell') {
            ...
            if (qtyR > ownedR) {
                Alert.alert('Insufficient Shares', `You only own ${sharesOwned.toFixed(2)}%. Cannot sell ${qty.toFixed(2)}%.`);
                return;
            }
        }
        if (qty * getUnitPrice() < 10) {
            Alert.alert('Minimum Trade', 'The minimum order value is $10.');
            return;
        }
                    ...
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.kav}>
                        <View style={styles.sheet}>
                            ...
                            <View style={styles.header}>
                                <Text style={styles.title}>{isBuy ? 'Buy Shares' : 'Sell Shares'}</Text>
                            ...
                            {orderType === 'market' && (
                                <View style={styles.marketRef}>
                                    <Text style={styles.marketRefLabel}>{isBuy ? 'Best Ask' : 'Best Bid'}</Text>
                            ...
                            >
                                {isSubmitting
                                    ...
                                    : <Text style={styles.confirmText}>{isBuy ? 'Confirm Purchase' : 'Confirm Sale'}</Text>
                                }
                            </TouchableOpacity>
