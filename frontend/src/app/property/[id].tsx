...
export default function PropertyScreen() {
    ...
    const handleConfirm = async (qty: number, type: OrderType, price?: number) => {
        try {
            ...
            Alert.alert('Success', `${type === 'limit' ? 'Limit order' : 'Market order'} placed successfully.`);
        } catch (error: any) {
        ...
        <>
            <PropertyDetails
                ...
                userHolding={userHolding ? {
                    sharesOwned: userHolding.shares_owned,
                    averagePrice: (userHolding as any).averagePrice ?? 0,
                } : undefined}
                onBack={() => router.back()}
